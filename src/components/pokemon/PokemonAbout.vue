...
const abilities = computed(() =>
  props.pokemon.abilities.filter(a => !a.is_hidden).map(a => capitalize(a.ability.name)).join(', ')
)

const genders = computed(() => {
  if (props.pokemon.sprites.front_default != null && props.pokemon.sprites.front_female != null) {
    return '♂ ♀'
  } else if (props.pokemon.sprites.front_default != null && props.pokemon.sprites.front_female == null) {
    return '♂'
  } else if (props.pokemon.sprites.front_female != null && props.pokemon.sprites.front_default == null) {
    return '♀'
  } else {
    return '—'
  }
  ...
  <div class="max-w-full">
    <div class="bg-white rounded-xl shadow p-4 text-left max-w-full">
      <div class="grid grid-cols-2 gap-3 text-sm text-gray-700">
        ...
        <div class="text-xs text-gray-400">Abilities</div>
